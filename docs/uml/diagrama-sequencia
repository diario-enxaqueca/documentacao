...
Este documento apresenta o **diagrama de sequência** do fluxo **Criar Entrada** no sistema **Diário de Enxaqueca**, detalhando a interação entre o cliente, API, controller, model e banco de dados.

---

## Diagrama (ASCII)

![Diagrama de Sequência](../assets/diagrama-sequencia-criar-entrada.png)
...
1. **Cliente**
   ...
   * Envia uma requisição HTTP `POST /entrada` com os dados da nova entrada de enxaqueca.
...
5. **Banco de Dados**
   ...
   * Insere o registro na tabela correspondente (`entradas`).
...
* Este fluxo ilustra o **CRUD “Criar Entrada”**, mas a lógica é similar para **Editar**, **Visualizar** e **Deletar**.
